...
void scheduler_steal::generate(FileDescription& file_desc, std::vector<std::string> nodes) {
	...
	for (int i = 0; i < nChunks; i++) {
		...
		physical_block.HBname    = file_desc.HB_blocks[i]; // added
		...
		physical_block.offset_in_file 	 = file_desc.offsets_in_file[i];  // added
		physical_block.huge_block_seq 	 = file_desc.huge_block_sequences[i];  // added
	...
	for(iter = lblocks_dist.begin(); iter != lblocks_dist.end() ; iter++){
		for(int i = 0; i<SLOTS; i++){
			...
			for(int id : replicas) {
				if( id != which_node){
					//iter->second[i].physical_blocks.insert(iter->second[i].physical_blocks.end(), lblocks_dist[id][i].physical_blocks.begin(), lblocks_dist[id][i].physical_blocks[lblocks_dist[id][i].assigned_num]);
					iter->second[i].physical_blocks.insert(iter->second[i].physical_blocks.end(), lblocks_dist[id][i].physical_blocks.begin(), lblocks_dist[id][i].physical_blocks.begin()+lblocks_dist[id][i].assigned_num);
				}
			}
			cout<< endl;
		}
