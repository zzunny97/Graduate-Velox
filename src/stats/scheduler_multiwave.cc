  ...
  for (auto& kv : slots_dist) {
    ...
    for (auto index : chunks_vec) {
      ...
      physical_block.node      = host;
  	  physical_block.HBname = fd.HB_blocks[index];
      physical_block.offset 	 = fd.offsets[index];  // added
      physical_block.offset_in_file 	 = fd.offsets_in_file[index];  // added
      physical_block.huge_block_seq 	 = fd.huge_block_sequences[index];  // added
