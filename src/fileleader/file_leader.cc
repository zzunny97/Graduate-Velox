...
bool FileLeader::file_insert_confirm(messages::FileInfo* f) {
  directory.file_table_confirm_upload(f->name, f->num_block, f->num_huge_block);
  directory.block_table_insert_all(f->blocks_metadata);

	for(auto& metadata : f->blocks_metadata){
		directory.small_block_table_insert_all(metadata.small_blocks, f->name);
	}
  return true;
}
// }}}
...
bool FileLeader::file_update(messages::FileUpdate* f) {
  if (file_exist(f->name)) {
    ...
    if (f->is_append) {
      BlockMetadata bi;
			SmallBlockMetadata sbi;
      directory.select_last_block_metadata(f->name, &bi);
      directory.small_select_last_block_metadata(bi.name, &sbi);
      //int last_seq = bi.seq;
      int last_seq = sbi.small_block_seq;

      /*for (auto& metadata : f->blocks_metadata) {
        metadata.seq = ++last_seq;
        directory.block_table_insert(metadata);
      }*/
			for (auto& metadata : bi.small_blocks) {
        metadata.small_block_seq = ++last_seq;
        directory.small_block_table_insert(metadata, f->name);
      }

      FileInfo fi;
      directory.file_table_select(f->name, &fi);
      directory.file_table_update(f->name, f->size + fi.size, last_seq + 1, fi.num_huge_block);

    } else {
      directory.file_table_update(f->name, f->size, f->num_block, f->num_huge_block);
      for (auto& metadata : f->blocks_metadata) {
        directory.block_table_insert(metadata);
				for(auto& Smetadata: metadata.small_blocks){
					directory.small_block_table_insert(Smetadata, f->name);
				}
      }
    }
...
bool FileLeader::file_delete(messages::FileDel* f) {
  if (file_exist(f->name)) {
    ...
    directory.small_block_table_delete_all(f->name);
    replicate_metadata();
...
shared_ptr<Message> FileLeader::file_request(messages::FileRequest* m) {
  ...
  directory.block_table_select(file_name, blocks);
	...
	fd->num_huge_block = blocks.size();
	uint32_t hblock_num = blocks.size();	

	std::vector< std::vector<SmallBlockMetadata> > sblock(hblock_num);
	//for(uint32_t i = 0; i < hblock_num; i++){
	uint32_t idx = 0 , end_hblocks = 0;
	uint32_t chunk_start_idx = 0;

	for(auto& block : blocks){
		directory.small_block_table_select(block.name, sblock[idx]);
		if(sblock[idx][0].small_block_seq == 0){
			int h_idx = GET_INDEX(block.hash_key);
			for(int i = 0; i< (int)hblock_num; i++){
	...
	if(fd->num_block != 0){
		for (uint32_t i = 0;  ; i++) {
			if(end_hblocks >= hblock_num)
				break;
			uint32_t end_iter = chunk_start_idx + hblock_num;
			for(uint32_t j = chunk_start_idx; j < end_iter ; j++) {
				idx = j % hblock_num;

				if(sblock[idx].size() > i){					
					fd->blocks.push_back(sblock[idx][i].name);
					fd->HB_blocks.push_back(blocks[idx].name);
					...
					fd->offsets_in_file.push_back(sblock[idx][i].offset_in_file);
					fd->small_block_sequences.push_back(sblock[idx][i].small_block_seq);
					fd->huge_block_sequences.push_back(sblock[idx][i].huge_block_seq);
