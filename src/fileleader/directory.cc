...
static int file_callback(void *file_info, int argc, char **argv, char **azColName) {
  ...
  file->num_huge_block     = atoi(argv[i++]);
...
static int small_block_callback(void *small_block_info, int argc, char **argv, char **azColName) {
  //int i = 0;
  int i = 1;
  auto block = reinterpret_cast<SmallBlockMetadata*>(small_block_info);
  block->name         = argv[i++];
  block->HBname    		= argv[i++];
  block->small_block_seq     = atoi(argv[i++]);
  block->size         = atoi(argv[i++]);
  block->offset				= atoll(argv[i++]);
  block->offset_in_file  = atoll(argv[i++]);
	block->huge_block_seq = atoi(argv[i++]);
...
static int file_list_callback(void *list, int argc, char **argv, char **azColName) {
  ...
  for (int i=0; i<argc; i++) {
    ...
    tmp_file.num_huge_block = atoi(argv[i++]);
...
static int small_block_list_callback(void *list, int argc, char **argv, char **azColName) {
  auto block_list = reinterpret_cast<vector<SmallBlockMetadata>*>(list);
  for (int i=1; i<argc; i++) {
    SmallBlockMetadata tmp_block;
//    tmp_block.file_name      = argv[i++];
    tmp_block.name      = argv[i++];
    tmp_block.HBname 		= argv[i++];
    tmp_block.small_block_seq  = atoi(argv[i++]);
    tmp_block.size      = atoi(argv[i++]);
    tmp_block.offset		= atoll(argv[i++]);
    tmp_block.offset_in_file = atoll(argv[i++]);
	tmp_block.huge_block_seq= atoi(argv[i++]);
...
void Directory::create_tables() {
  ...
  if (query_exec_simple(sql))
	...
	sprintf(sql, "CREATE TABLE IF NOT EXISTS small_block_table( \
			file_name 		TEXT 			NOT NULL, \
      name          TEXT      NOT NULL, \
      HBname				TEXT      NOT NULL, \
      small_block_seq      INT       NOT NULL, \
      size          INT       NOT NULL, \
      offset        INT       NOT NULL, \
      offset_in_file		INT       NOT NULL, \
			huge_block_seq INT 			NOT NULL, \
      PRIMARY KEY (name));"); 
...
void Directory::file_table_insert (FileInfo &file_info) {
  ...
  sprintf(sql, "INSERT INTO file_table (\
    name, hash_key, size, num_block, num_huge_block, n_lblock, type, replica, uploading, is_input, intended_block_size)\
    VALUES('%s', %" PRIu32 ", %" PRIu64 ", %u, %u, %u, %u, %u, %u, %u, %" PRIu64 ");",
      file_info.name.c_str(),
      file_info.hash_key,
      file_info.size,
      file_info.num_block,
      file_info.num_huge_block,
      file_info.n_lblock,
      file_info.type,
      file_info.replica,
      file_info.uploading,
...
// }}}
// file_table_update {{{
void Directory::file_table_update(string file_name, uint64_t size, uint32_t num_block, uint32_t num_huge_block) {
  char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "UPDATE file_table set \
      size=%" PRIu64 ", num_block=%u, num_huge_block=%u where name='%s';",
      size, num_block, num_huge_block, file_name.c_str());
...
void Directory::file_table_confirm_upload (std::string file_name, uint32_t num_block, uint32_t num_huge_block) {
  char sql[DEFAULT_QUERY_SIZE];
  sprintf(sql, "UPDATE file_table set uploading=0, num_block=%u, num_huge_block=%u where name='%s';", num_block, num_huge_block,
      file_name.c_str());
...
void Directory::small_block_table_insert(SmallBlockMetadata& metadata, std::string file_name){
	char sql[DEFAULT_QUERY_SIZE];
	
/*	printf("INSERT OR REPLACE INTO small_block_table \
    ['%s', '%s', '%s', %u,  %llu,  %llu, %llu, %u ];\n",
      file_name.c_str(),
			metadata.name.c_str(),
      metadata.HBname.c_str(),
      metadata.small_block_seq,
      metadata.size,
      metadata.offset,
      metadata.offset_in_file,
			metadata.huge_block_seq);
*/
  sprintf(sql, "INSERT OR REPLACE INTO small_block_table (\
    file_name, name, HBname, small_block_seq,  size, offset, offset_in_file, huge_block_seq)\
...
void Directory::small_block_table_insert_all(std::vector<SmallBlockMetadata>& metadata, std::string file_name){
	char *zErrMsg = nullptr;
  sqlite3* db = nullptr;
  sqlite3_stmt * stmt;
  const char * tail = 0;
  ...
  char sql[DEFAULT_QUERY_SIZE];
  sprintf(sql, "INSERT OR REPLACE INTO small_block_table VALUES (@FNAME, @NAME, @HBNAME, @SEQ,  @SZ , @OFFSET, @FOFFSET, @HBSEQ);");
  sqlite3_prepare_v2(db, sql, DEFAULT_QUERY_SIZE, &stmt, &tail);
  sqlite3_exec(db, "BEGIN TRANSACTION", NULL, NULL, &zErrMsg);
  ...
  for (auto& block : metadata) {
    ...
    sqlite3_bind_text  (stmt, 3, block.HBname.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_int   (stmt, 4, block.small_block_seq);
    sqlite3_bind_int64 (stmt, 5, block.size);
    sqlite3_bind_int64 (stmt, 6, block.offset);
    sqlite3_bind_int64 (stmt, 7, block.offset_in_file);
    sqlite3_bind_int   (stmt, 8, block.huge_block_seq);
...
void Directory::small_block_table_select(std::string HBname, std::vector<SmallBlockMetadata>& blocks){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "SELECT * from small_block_table where (HBname ='%s');", HBname.c_str());

  query_exec_simple(sql, small_block_list_callback, (void*)&blocks);

}
void Directory::small_block_table_select_by_index(std::string HBname, uint32_t block_seq, SmallBlockMetadata* block_info){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "SELECT * from small_block_table where (HBname='%s') and \
      (small_block_seq=%u);", HBname.c_str(), block_seq);

  query_exec_simple(sql, small_block_callback, (void*)block_info);
}

void Directory::small_block_table_select_all(std::vector<SmallBlockMetadata>& block_info){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "SELECT * from small_block_table;");
  query_exec_simple(sql, small_block_list_callback, (void*)&block_info);
}

void Directory::small_block_table_update(std::string HBname, uint32_t size, uint32_t seq){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "UPDATE block_table set \
      size=%" PRIu32 " where (HBname='%s') and (seq=%u);",
      size, HBname.c_str(), seq);

  if (query_exec_simple(sql))
    DEBUG("block_metadata updated successfully");

}
void Directory::small_block_table_delete(std::string HBname, uint32_t seq){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "DELETE from small_block_table where (HBname='%s') and (small_block_seq=%u);", HBname.c_str(), seq);
  if (query_exec_simple(sql))
    DEBUG("block_metadata deleted successfully");

}
void Directory::small_block_table_delete_all(std::string File){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "DELETE from small_block_table where (file_name='%s');", File.c_str());
  if (query_exec_simple(sql))
    DEBUG("block_metadata deleted successfully");

}
void Directory::small_select_last_block_metadata(std::string HBname, SmallBlockMetadata* block_info){
	char sql[DEFAULT_QUERY_SIZE];

  sprintf(sql, "SELECT * FROM block_table WHERE (HBname='%s') \
      ORDER BY small_block_seq DESC LIMIT 1;", HBname.c_str());

  if (query_exec_simple(sql, small_block_callback, (void*)block_info))
    DEBUG("The last block_metadata selected successfully");
