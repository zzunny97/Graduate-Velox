...
JNIEXPORT jint JNICALL Java_com_dicl_velox_VeloxDFS_readChunk
  (JNIEnv* env, jobject obj, jstring chunk_name, jstring host, jbyteArray buf, jint boff, jint off, jlong len, jlong lbm_id) {
  ...
  int32_t readBytes = vdfs->read_chunk(chunk_name_, host_, c_buf, boff, (uint32_t)off, (uint64_t)len, lbm_id);
...
JNIEXPORT jintArray JNICALL Java_com_dicl_velox_VeloxDFS_getCurChunkIdx
  (JNIEnv *env, jobject obj, jint lbm_id) {

  	velox::vdfs* vdfs = get_vdfs(env, obj);
	jintArray cidx = env->NewIntArray(2);
	jint *Ret = new jint[2];

	int* ret = vdfs->get_current_chunk_index((int)lbm_id);
