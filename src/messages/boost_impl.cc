...
template <typename Archive>
  void serialize (Archive& ar, eclipse::messages::FileInfo& c, unsigned int) {
    ...
    ar & BOOST_SERIALIZATION_NVP(c.num_huge_block);
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::messages::BlockInfo& c, unsigned int) {
    ar & BASE_OBJECT(Message, c);
    ar & BOOST_SERIALIZATION_NVP(c.name); 
    ar & BOOST_SERIALIZATION_NVP(c.HBname); 
    ...
    ar & BOOST_SERIALIZATION_NVP(c.type);   
    ar & BOOST_SERIALIZATION_NVP(c.replica);   
    ar & BOOST_SERIALIZATION_NVP(c.huge_block_seq);   
    ar & BOOST_SERIALIZATION_NVP(c.offset);   
    ar & BOOST_SERIALIZATION_NVP(c.offset_in_file);   
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::messages::FileDescription& c, unsigned int) {
    ...
    ar & BOOST_SERIALIZATION_NVP(c.HB_blocks);
    ...
    ar & BOOST_SERIALIZATION_NVP(c.small_block_sequences);
    ar & BOOST_SERIALIZATION_NVP(c.huge_block_sequences);
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::SmallBlockMetadata& c, unsigned int) {
    ar & BOOST_SERIALIZATION_NVP(c.name); 
    ar & BOOST_SERIALIZATION_NVP(c.HBname);  
    ar & BOOST_SERIALIZATION_NVP(c.small_block_seq);  
    ar & BOOST_SERIALIZATION_NVP(c.size);     
    ar & BOOST_SERIALIZATION_NVP(c.offset);   
    ar & BOOST_SERIALIZATION_NVP(c.offset_in_file);   
    ar & BOOST_SERIALIZATION_NVP(c.huge_block_seq);   
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::BlockMetadata& c, unsigned int) {
    ...
    ar & BOOST_SERIALIZATION_NVP(c.r_node);      
    ar & BOOST_SERIALIZATION_NVP(c.is_committed);  
    ar & BOOST_SERIALIZATION_NVP(c.small_blocks);  
  }
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::messages::IOoperation& c, unsigned int) {
    ...
    ar & BOOST_SERIALIZATION_NVP(c.assigned_chunks);
    ar & BOOST_SERIALIZATION_NVP(c.lbm_id);
    ar & BOOST_SERIALIZATION_NVP(c.retry);
    ar & BOOST_SERIALIZATION_NVP(c.cidx);
  }
...
template <typename Archive>
  void serialize (Archive& ar, eclipse::logical_block_metadata& c, unsigned int) {
    ...
    ar & BOOST_SERIALIZATION_NVP(c.size);
    ar & BOOST_SERIALIZATION_NVP(c.hash_key);
    ar & BOOST_SERIALIZATION_NVP(c.assigned_num);
...
template void serialize (boost::archive::xml_oarchive&, eclipse::SmallBlockMetadata&, unsigned);
template void serialize (boost::archive::xml_iarchive&,  eclipse::SmallBlockMetadata&, unsigned);
template void serialize (boost::archive::binary_iarchive&,  eclipse::SmallBlockMetadata&, unsigned);
template void serialize (boost::archive::binary_oarchive&,  eclipse::SmallBlockMetadata&, unsigned);
...
BOOST_CLASS_EXPORT_IMPLEMENT(eclipse::SmallBlockMetadata);
